...
bash "/app/telegram_bot/wait_for_model.sh"
WAIT_RESULT=$?

echo "Результат ожидания: $WAIT_RESULT (0 = успех)"

# Если скрипт ожидания успешно завершился, запускаем бот
if [ $WAIT_RESULT -eq 0 ]; then
    # Сначала восстанавливаем базы данных
    echo "Восстанавливаем базы данных..."
    cd /app
    make restore-db || echo "Предупреждение: Не удалось восстановить базы данных"
    
    # Затем скачиваем файлы
    echo "Скачиваем файлы..."
    make download-files || echo "Предупреждение: Не удалось скачать файлы"
